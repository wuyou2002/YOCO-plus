...
prompt_text_8k = prompt_text_4096 * 2
prompt_text_16k = prompt_text_8k * 2
prompt_text_32k = prompt_text_16k * 2
prompt_text_64k = prompt_text_32k * 2
...
def prepare(method: str, cpu_offload: str = "none"):
    """
    Prepare the tokenizer and model.

    Arguments:
        method (str): The model name. Options are "llama", "yoco", "fusedkv-lite", "fusedkv" and "yoco-plus".
        cpu_offload (str): The CPU offload strategy. Options are "none", "hf", and "ds".
    """

    CONFIG_MAPPING = {
        "llama": "configs/tinyllama.json",
        "yoco": "configs/tinyllama_yoco.json",
        "fusedkv-lite": "configs/tinyllama_fusedkv_lite.json",
        "fusedkv": "configs/tinyllama_fusedkv.json",
        "yoco-plus": "configs/tinyllama_yoco_plus.json",
    }
    ...
    # prepare config
    config = AutoConfig.from_pretrained(CONFIG_MAPPING[method])
    ...
    if cpu_offload == "none":
        ...
        model = get_hf_model(
            model_name = CONFIG_MAPPING[method],
            config = config,
...
def experiment(tokenizer, model, prompt, max_new_tokens, iterator=None, verbose=False):
    ...
    def run(num_return_sequences = 32):
        ...
        model.generate(
            ...
            min_length=max_new_tokens+input_ids.shape[1],
            temperature=1.0,
            top_k=0,
            top_p=0.9,
...
def main():
    ...
    print(">>> prefill 8k llama")
    tokenizer, model = prepare("llama")
    experiment(tokenizer, model, prompt_text_8k, max_new_tokens=1)

    print(">>> prefill 8k yoco-plus")
    tokenizer, model = prepare("yoco-plus")
    experiment(tokenizer, model, prompt_text_8k, max_new_tokens=1)

    print(">>> decoding 8k llama")
    tokenizer, model = prepare("llama")
    experiment(tokenizer, model, prompt_text_5, max_new_tokens=8192)

    print(">>> decoding 8k yoco-plus")
    tokenizer, model = prepare("yoco-plus")
    experiment(tokenizer, model, prompt_text_5, max_new_tokens=8192)
