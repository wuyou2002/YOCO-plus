...
def apply_liger_kernel_to_xkv_llama(
    rope: bool = True,
    cross_entropy: bool = False,
    fused_linear_cross_entropy: bool = True,
    rms_norm: bool = True,
    ...
    """
    Apply Liger kernels to replace original implementation in XKV models.
    ...
    from . import modeling_xkv

    if rope:
        modeling_xkv.apply_rotary = liger_rotary
    if rms_norm:
        modeling_llama.LlamaRMSNorm = LigerRMSNorm
    if swiglu:
        modeling_llama.LlamaMLP = LigerSwiGLUMLP
    if cross_entropy:
        modeling_llama.CrossEntropyLoss = LigerCrossEntropyLoss
    if fused_linear_cross_entropy:
        modeling_xkv.LlamaForCausalLM.forward = llama_lce_forward
